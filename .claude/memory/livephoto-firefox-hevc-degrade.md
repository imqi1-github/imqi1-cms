...
**修复(前端降级,已生效):** LivePhoto.vue `onVideoError` + useFancyboxLivePhoto.ts `handleVideoError` 监听 video `@error`,失败清 videoBlobUrl/移除 DOM 退化为静态图。@error 不挑根因,HEVC 或切片错都兜底。用户 2026-07-23 确认接受 Firefox 全部降级、不要转码。
