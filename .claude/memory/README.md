...
## ⚠️ 双份关系（重要）
Claude Code 的 auto-memory **每会话自动加载的路径写死在用户级**：
`C:\Users\Administrator\.claude\projects\A--nodejs-imqi1\memory\`

- **本项目 `.claude/memory/` = 权威/存储**（进 git、可分享）
- **用户级那份 = 自动加载的镜像**（harness 只从那里读）

改动某条记忆时**两边同步写**（本项目先写，再复制到用户级路径）。只改一边会导致：仓库里有、但本会话自动记忆不更新，或反之。
