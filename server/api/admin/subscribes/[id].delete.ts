...
import { CSRF_HEADER, SUBSCRIBE_CACHE_ROUTES  } from "#shared/constants";
...
export default defineEventHandler(async event => {
  ...
  try {
    ...
    void invalidateContentCaches({ routes: SUBSCRIBE_CACHE_ROUTES }).catch(err => console.error("[cache] 订阅删除失效缓存失败", err));
    return { success: true };
  } catch (error) {
