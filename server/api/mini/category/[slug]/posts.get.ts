...
import type { MiniCategoryPost, MiniCategoryPostsResponse } from "#server/types/apis/mini";
...
export default defineEventHandler(async event => {
  ...
  try {
    ...
    const total = await prisma.postrelations.count({ where: postWhere });
    ...
    const posts: MiniCategoryPost[] = relations.map(relation => {
      const post = relation.posts;

      const attachmentMetadata = post.attachments.map(item => ({
        keys: buildUrlKeys(item.attachment.url),
        metadata: normalizeAttachmentMetadata(item.attachment.metadata),
      }));

      const firstCover = parseCovers(post.covers)[0];
      const coverCount = parseCovers(post.covers).length;
    ...
    } satisfies MiniCategoryPostsResponse;
  } catch (error) {
