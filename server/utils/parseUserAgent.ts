...
/**
 * 解析 User Agent 字符串，返回浏览器和操作系统信息（服务端版）。
 *
 * 公开评论接口不再向前端下发原始 User Agent，改由服务端预解析成展示用的
 * browser/os 图标信息，避免泄露访客的浏览器指纹。逻辑与前端
 * app/utils/parseUserAgent.ts 保持一致。
 */
...
export function parseUserAgent(userAgent: string): ParsedAgent {
  ...
  let browserIcon = "ri-computer-line";
  let os: string | null = null;
  let osIcon = "ri-smartphone-line";
