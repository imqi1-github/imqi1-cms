 ...
 *
 * 【精确到页】调用方按"本次改了什么 → 影响哪些页面"传 routes，取值一律用 shared/constants.ts
 * 里那组具名常量（LINKS_CACHE_ROUTES / SUBSCRIBE_CACHE_ROUTES / CONTENT_CACHE_ROUTES…），
 * 别就地手写数组 —— 同一组原先在几十个接口里各抄一份，改一处漏一处前台就一直看到旧数据。
