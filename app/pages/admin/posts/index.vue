      ...
      <Table v-else class="hidden min-[1175px]:table">
        ...
        <TableBody>
          <TableRow v-for="post in posts" :key="post.cid" :class="{ 'bg-muted/50': selectedIds.includes(post.cid) }">
            ...
            <TableCell>
              <div v-if="post.postrelations && post.postrelations.length > 0" class="flex flex-wrap gap-1">
                <Badge v-for="rel in post.postrelations" :key="rel.metas.mid" variant="outline" class="text-xs">
                  {{ rel.metas.name }}
                </Badge>
              </div>
