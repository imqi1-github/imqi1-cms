...
function resizeElements (elements) {
	...
	infos.forEach(info => {
		...
		info.element.maxHeight = 15 * oneLinerHeight + 12;
...
const Self = {
	...
	effect (Prism) {
		...
		const completeHook = Prism.hooks.add('complete', env => {
			...
			if (pre.getBoundingClientRect().height >= pre.maxHeight) {
				pre.style.maxHeight = pre.maxHeight + "px";
				let div = document.createElement('div');
				div.innerHTML = "<i class=\"ri-more-fill\"></i>"
				div.className = "code-expand";
				Prism.bindEvent(div, 'click', () => {
					pre.removeChild(div);
					pre.style.maxHeight = null;
					Prism.hooks.run("line-numbers", pre);
				})
				pre.appendChild(div);
			}
		});
